...
class Moving(Board):
    ...
    def piece_xy(self, mouse):
        ...
        self.mousex = mouse[0]
        self.mousey = mouse[1]
        self.col = self.mousex // self.square_size #column of mouse click
        self.row = self.mousey // self.square_size #row of mouse click
        self.xy = self.row, self.col # xy on board
    ...
    def sideturn(self, validmove):
        ...
        self.turn = getattr(self, "turn", "white") #setting turn initally as white
        if validmove == True: #if move was valid
            if len(self.selectlist) == 2: #if length of list is two
                self.firstitem = self.selectlist[0]
                #swaps turn if white now black, if black now white
                #if first piece moved was also white
    ...
    def movepos(self, validmove):
        self.validmove = validmove
        #if length of list two, move was true and was valid
        if len(self.selectlist) == 2 and self.move == True and self.validmove == True:
    ...
    def co_ords(self):
        #coordinates of the mouse clicks
